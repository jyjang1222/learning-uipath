...
- 값을 잘 못가져와서 사용하는 Delay 액티비티를 대체할 수 있다.
- 속성
    - 인터벌 다시 시도(RetryInterval) : 시퀀스를 재시도할 횟수이다.
    - 재시도 횟수(NumberOfRetries) : 각 재시도 사이의 시간(초)을 지정한다.

### Get Credential
- 제공된 AssetName 을 사용하여 지정된 Orchestrator 자격 증명을 가져오고 사용자 이름과 보안 암호를 반환합니다.
- 속성
    - 오케스트레이터 폴더 경로 : 왼쪽 폴더 패널에서 확인가능하고 폴더 이름을 입력
    - Asset Name : 오케스트레이터 Asset탭에서 가져오고자 하는 계정 이름
    - 사용자 이름 : 메일을 보낼때 로그온에 사용되는 이메일 계정ID 반환 (SMTP)
    - 암호 : 메일을 보낼때 로그온에 사용되는 이메일 계정PW 반환 (SMTP)

### Download Storage File
- 오케스트레이터 스토리지 버킷에 있는 파일을 받아올 때 사용된다.
- 속성
    - 경로 : 받아올 파일 이름을 입력한다.
    - 스토리지 버킷 이름 : 필요한 파일이 들어있는 스토리지 버킷 이름을 입력한다.

### Create HTML Content
- 메일 본문에 넣을 이메일틀을 작성 할 수 있다.